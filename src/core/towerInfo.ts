...
import { sniperExecuteEffect } from '../data/sniperMergeEffects';
...
function noteOf(key: string, level: number, stats: TowerLevelStats, attack: string): string {
  ...
  if (key === 'sniper') {
    const execute = sniperExecuteEffect(level);
    return execute
      ? `처형: HP ${Math.round(execute.healthRatio * 100)}%↓ ${execute.damageMultiplier}배`
      : '';
  }
  return '';
}
