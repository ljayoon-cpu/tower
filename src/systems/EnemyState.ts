...
export class EnemyState {
  ...
  /** 번개 경직은 같은 적에게 연달아 걸리지 않도록 재발동 시간을 둔다. */
  applyStagger(durationMs: number, cooldownMs: number): boolean {
    if (this.staggerCooldownLeftMs > 0) return false;
    this.staggerLeftMs = Math.max(0, durationMs);
  ...
  update(dtMs: number): number {
    ...
    if (this.shield >= this.maxShield || this.maxShield === 0 || dt === 0) return staggeredForMs;
