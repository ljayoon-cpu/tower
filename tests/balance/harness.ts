...
import { waveInterestRate, WAVE_INTEREST_CAP } from '../../src/core/constants';
...
export function simulate(
  ...
  game.bus.on('wave:cleared', () => {
    ...
    game.eco.applyInterest(waveInterestRate(game.waves.waveIndex + 1), WAVE_INTEREST_CAP); // Game.create wires the same
 ...
 */
export function monoTower(key: string, path?: 'a' | 'b'): Strategy {
  ...
  return c => {
    for (let guard = 0; guard < 400; guard++) {
      ...
      if (lowestPair) { c.merge(lowestPair[1][0], lowestPair[1][1], path); continue; }
...
export function mergeArmy(keys: string[], maxLevel = 3, path?: 'a' | 'b'): Strategy {
  return c => {
    ...
    for (let level = 2; level <= maxLevel; level++) for (const tower of core) {
      ...
      const make = (targetLevel: number): Tower | null => {
        ...
        while (result.level < targetLevel) {
          ...
          c.merge(feeder, result, path);
        }
        return result;
      };
      const feeder = make(tower.level);
      if (feeder) c.merge(feeder, tower, path);
    }
    spread(keys)(c);
  };
}
