...
- [x] 일시정지 오버레이에 현재 타워 구성 요약 (`core/towerRoster`)

### C. 콘텐츠 (중간)
- [x] 스테이지 1-7 (3갈래 분기), 1-8 (트렁크 후 중첩 분기 → 출구 4개). 둘 다 `tests/balance` 회귀 통과.
- [x] 새 타워 2종 — **저격탑**(sniper, `cd952c3`), **독 타워**(poison DoT, `4fe7c5d`). 둘 다 Codex 작업.
- [ ] 새 적 1종 (예: 방어막/장갑/분열) — `EnemyDef`에 필드 추가. **주의: `Enemy.ts`·`types.ts`·
      `textures.ts`를 Codex가 자주 만짐 → Codex와 겹침. 시작 전 조율.**
- [ ] 월드 2 (2-1~) — 새 타일 팔레트/맵 테마

### D. 깊이 (중간)
- [ ] 웨이브 사이 이자(interest) — 저축 유인. **밸런스 파급 큼**: 골드가 늘어 8스테이지
      전부 재조정 필요. `tests/balance/harness.ts`의 `wave:cleared` 핸들러에도 이자를 넣어야
      시뮬이 정확해짐.
